    ...
    public partial class frmProductList : Form
    {
        ...
        public void LoadRecords()
        {
            ...
            using (SqlConnection sqlConnection=new SqlConnection(DBConnection.MyConnection()))
            {
                int i = 0;
                sqlConnection.Open();
                string query = @"select p.pcode,p.pdesc,b.brand,c.category,p.price,p.qty from tblProduct as p
        ...
        private void grvData_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (colName=="Edit")
                {
                    ...
                    frmProduct.txtDescription.Text = grvData.Rows[e.RowIndex].Cells[2].Value.ToString();
                    frmProduct.txtPrice.Text = grvData.Rows[e.RowIndex].Cells[5].Value.ToString();
                    frmProduct.cmbBrand.Text = grvData.Rows[e.RowIndex].Cells[3].Value.ToString();
                    frmProduct.cmbCategory.Text = grvData.Rows[e.RowIndex].Cells[4].Value.ToString();
